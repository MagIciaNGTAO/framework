 ...
 * created and each time the {@code #fancyName(String)} throws an exception of
 * type {@code cause} (or a subclass), the meter will be marked.
 * <p>
 * A name for the metric can be specified as an annotation parameter, otherwise,
 * the metric will be named based on the method name.
 * <p>
 ...
 * }</pre>
 * <p>
 * A meter named {@code fancyName.exceptions} will be created and marked every
...
public @interface ExceptionMetered {
     ...
     */
    Class<? extends Throwable> cause() default Throwable.class;
}
